...
func TestQueryLogProtoFilter(t *testing.T) {
	store, err := NewQueryLogStore(filepath.Join(t.TempDir(), "querylog.db"))
	if err != nil {
		t.Fatalf("NewQueryLogStore: %v", err)
	}
	...
	now := time.Now()
	for _, e := range []QueryLogEntry{
		{Timestamp: now, Instance: "a", Client: "c", Domain: "doh.test", Action: "PASS", Proto: "doh"},
		{Timestamp: now, Instance: "a", Client: "c", Domain: "dns.test", Action: "PASS", Proto: "dns"},
	} {
		if err := store.Insert(ctx, e); err != nil {
			t.Fatalf("Insert: %v", err)
		}
	...
	for _, tc := range []struct {
		...
		{"", 2}, {"doh", 1}, {"dns", 1}, {"bogus", 2},
	} {
		entries, err := store.Query(ctx, "", "", "", "", tc.proto, now.Add(-time.Hour), 0, 100)
		if err != nil {
			t.Fatalf("Query proto=%q: %v", tc.proto, err)
		}
		if len(entries) != tc.want {
			t.Errorf("Query proto=%q returned %d entries, want %d", tc.proto, len(entries), tc.want)
		}
		if tc.proto == "doh" || tc.proto == "dns" {
			for _, e := range entries {
				if e.Proto != tc.proto {
					t.Errorf("Query proto=%q returned entry with proto=%q", tc.proto, e.Proto)
				}
			}
		}
		n, err := store.QueryCount(ctx, "", "", "", "", tc.proto, now.Add(-time.Hour))
		if err != nil {
			t.Fatalf("QueryCount proto=%q: %v", tc.proto, err)
		}
		if n != tc.want {
