...
func KeyOf(m *dns.Msg) Key {
	...
	q := m.Question[0]
	return KeyOfNormalized(q.Name, q.Qtype, q.Qclass)
}

// KeyOfNormalized returns the cache key for an already-lowercased FQDN plus
// type/class. Use it when the caller already normalized the qname (e.g. the
// DNS serve path) to avoid a second strings.ToLower allocation per query.
func KeyOfNormalized(fqdn string, qtype, qclass uint16) Key {
	...
	return Key{Name: lower, QType: qtype, QClass: qclass}
}
